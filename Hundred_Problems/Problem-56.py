...
# def convert_currency(amount: float, from_currency: str, to_currency: str) -> float:
#     currencies = {"USD": 1.0, "EUR": 0.85, "GBP": 0.75, "JPY": 110.0, "THB": 32.0}
#     return (amount * currencies[from_currency]) * currencies[to_currency] if (from_currency == "USD") else round(amount * (currencies[to_currency] / currencies[from_currency]), 2)

def convert_currency(amount: float, from_currency: str, to_currency: str) -> float:
    currencies = {"USD": 1.0, "EUR": 0.85, "GBP": 0.75, "JPY": 110.0, "THB": 32.0}
    return round(amount * currencies[to_currency.upper()] / currencies[from_currency.upper()], 2)

if __name__ == "__main__":
    amount, from_currency, to_currency = 100, "USD", "EUR"
    print(convert_currency(amount, from_currency, to_currency))