...
class LibraryBook:
    def __init__(self, title: str, author: str, year_published: int, isbn: str):
        self.title, self.author, self.year_published, self.isbn, self.status = title, author, year_published, isbn, "available"

    def check_out(self):
        self.status = "checked out"
        print("Checked out  success.")
...
if __name__ == "__main__":
    library = []
    while True:
        select = input("1. Add a new book\n2. Check out a book\n3. Return a book\n4. Display information about a specific book\n5. Display all books in the library\n6. Exit\nSelect: ")
        match select:
            case "1": library.append(LibraryBook(input("\nEnter title name: "), input("Enter author: "), int(input("Enter year published: ")), input("Enter isbn: ")))
            case "2":
                title_name = input("\nEnter title name: ")
                for book in library:
            ...
            case "3":
                title_name = input("\nEnter title name: ")
                for book in library:
                    if title_name == book.return_book():
                        book.check_out()
                        break
            case "4":
                title_name = input("\nEnter title name: ")
                for book in library:
                    if title_name == book.title:
                        ...
                        break
            case "5":
                for book in library: book.display_info()
