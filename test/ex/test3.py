    ...
    match choice:
        ...
        case 2:
            ...
            for i in range(8):
                ...
                if net != 0 :print(f"|{net:>{len("          Net  ")},.2f} - {income:>{len("  Income    ")},.2f} |   {tr:2}%  | {tax:<{len("                Tax  ")}} |{price:>{len("               ")},.2f} |")
                else: print(f"|{"":>{len("          Net  ")}} > {income:>{len("  Income    ")},.2f} |   {tr:2}%  | {tax:<{len("                Tax  ")}} |{price:>{len("               ")},.2f} |")
                if net_income == 0: break
            print(l,f"|{"Total Tax":^{len("           1.00 -   150,000.00 |    0%  | -100,000.00 * 0.00    ")}}|{total:>{len("               ")},.2f} |",l,sep="\n")
    print()